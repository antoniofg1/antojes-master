...
class PrivateChatController extends AbstractController
{
    ...
    public function cambiarChat(
        ...
        if (!$data || !isset($data['chat_id'])) {
            return new JsonResponse(['data' => null, 'error' => 'chat_id required'], 400);
        }
        ...
        if (!$chat) {
            return new JsonResponse(['data' => null, 'error' => 'Chat not found'], 404);
        }
        ...
        if (!$member) {
            return new JsonResponse(['data' => null, 'error' => 'Not a member of this chat'], 403);
        }
    ...
    public function salir(
        ...
        if (!$chat || $chat->getType() !== 'private') {
            return new JsonResponse(['data' => null, 'error' => 'Chat not found or not private'], 404);
        }
        ...
        return new JsonResponse(['data' => ['message' => 'Left chat'], 'error' => null]);
    }
}
